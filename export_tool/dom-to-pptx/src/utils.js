 ...
 */
export function extractTableData(node, scale, measureScaleX = 1) {
  const rows = [];
  const colWidths = [];
  ...
  if (firstRow) {
    ...
    cells.forEach((cell) => {
      ...
      const wIn = rect.width * measureScaleX * (1 / 96) * scale;
      colWidths.push(wIn);
    });
  }
...
export function isTextContainer(node) {
  ...
  const hasText =
    node.textContent.trim().length > 0 || readPseudo('::before') || readPseudo('::after');
  if (!hasText) return false;

  // Treat icon elements as non-text containers (rendered as images elsewhere).
  const nodeCls = (node.getAttribute('class') || '').toLowerCase();
  if (
    nodeCls.includes('fa-') ||
    nodeCls.includes('fas') ||
    nodeCls.includes('far') ||
    nodeCls.includes('fab') ||
    nodeCls.includes('material-icons') ||
    nodeCls.includes('material-symbols') ||
    nodeCls.includes('bi-') ||
    nodeCls.includes('icon')
  ) {
    return false;
  }
