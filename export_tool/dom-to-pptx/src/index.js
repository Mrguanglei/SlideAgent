...
function getTextBoxExpandIn(config, globalOptions = {}, text, style) {
  const baseRaw =
    typeof globalOptions.textBoxExpandPx === 'number'
      ? globalOptions.textBoxExpandPx
      : typeof globalOptions.textBoxExpand === 'number'
        ? globalOptions.textBoxExpand
        : 0;
  const basePx = Number.isFinite(baseRaw) ? baseRaw : 0;

  const mode =
    globalOptions.textBoxExpandMode ||
    (typeof globalOptions.textBoxExpandPx === 'number' ? 'fixed' : 'auto');

  if (mode === 'fixed') return basePx * PX_TO_INCH * config.scale;

  let textStr = '';
  if (Array.isArray(text)) {
    textStr = text.map((t) => (t && typeof t.text === 'string' ? t.text : '')).join('');
  } else if (typeof text === 'string') {
    textStr = text;
  }

  const fontPx = style ? parseFloat(style.fontSize) || 0 : 0;
  const cjkFactor = Number.isFinite(globalOptions.textBoxExpandCjkFactor)
    ? globalOptions.textBoxExpandCjkFactor
    : 0.12;
  const latinFactor = Number.isFinite(globalOptions.textBoxExpandLatinFactor)
    ? globalOptions.textBoxExpandLatinFactor
    : 0.06;
  const autoPx = fontPx > 0 ? fontPx * (hasCjk(textStr) ? cjkFactor : latinFactor) : 0;
  const finalPx = Math.max(basePx, autoPx, 0);
  return finalPx * PX_TO_INCH * config.scale;
}
 ...
 * @param {string} [options.layout='LAYOUT_WIDE'] - PptxGenJS layout name (e.g. LAYOUT_16x9, LAYOUT_WIDE)
 * @param {boolean} [options.waitForFonts=true] - Wait for document fonts before capture
 * @param {string} [options.iconBaseUrl] - Base URL for icon images (e.g. '/demo/icons')
 * @param {string} [options.iconExt='svg'] - Extension for icon images (svg/png)
 * @param {string} [options.iconPathTemplate] - Template for icon URL, supports {base} {name} {ext}
 * @param {Object} [options.iconMap] - Map of icon name -> URL
 * @param {Function} [options.iconResolver] - (iconName, node) => URL
...
export async function exportToPptx(target, options = {}) {
  // Ensure web fonts (e.g. Material Icons) are loaded before rendering to canvas
  if (options.waitForFonts !== false && document?.fonts?.ready) {
    try {
  ...
  const layoutName = options.layout || 'LAYOUT_WIDE';
  if (layoutName === 'LAYOUT_WIDE') {
    // Force true 13.333" x 7.5" layout even if defaults differ
    const forcedLayout = 'DOM_TO_PPTX_WIDE';
    pptx.defineLayout({ name: forcedLayout, width: 13.333, height: 7.5 });
    pptx.layout = forcedLayout;
  } else {
  ...
  if (fontsToEmbed.length > 0) {
    ...
    const resolveFontType = (fontCfg) => {
      const raw = (fontCfg.type || fontCfg.format || '').toString().toLowerCase();
      if (raw) return raw;
      if (fontCfg.url) {
        return fontCfg.url.split('.').pop().split(/[?#]/)[0].toLowerCase();
      }
      return 'ttf';
    };

    const resolveFontBuffer = async (fontCfg) => {
      if (fontCfg.data instanceof ArrayBuffer) return fontCfg.data;
      if (fontCfg.buffer instanceof ArrayBuffer) return fontCfg.buffer;
      if (fontCfg.blob instanceof Blob) return fontCfg.blob.arrayBuffer();
      if (!fontCfg.url) throw new Error('Font source missing (url/data).');
    ...
    for (const fontCfg of fontsToEmbed) {
      try {
        const buffer = await resolveFontBuffer(fontCfg);

        let type = resolveFontType(fontCfg);
        if (type === 'woff2') {
          console.warn(`Skipping woff2 font (unsupported): ${fontCfg.name}`);
          continue;
        }
        if (!['woff', 'otf', 'ttf'].includes(type)) type = 'ttf';

        await embedder.addFont(fontCfg.name, buffer, type);
      } catch (e) {
        const srcLabel = fontCfg.url || fontCfg.name || 'unknown source';
        console.warn(`Failed to embed font: ${fontCfg.name} (${srcLabel})`, e);
      }
    }
...
async function processSlide(root, slide, pptx, globalOptions = {}) {
  ...
  const cssWidth = parseFloat(rootStyle.width) || 0;
  const cssHeight = parseFloat(rootStyle.height) || 0;
  const rootWidthPx =
    cssWidth ||
    root.offsetWidth ||
    root.scrollWidth ||
  ...
  const rootHeightPx =
    ...
    root.scrollHeight ||
    rootRect.height;
  const measureScaleX = rootRect.width ? rootWidthPx / rootRect.width : 1;
  const measureScaleY = rootRect.height ? rootHeightPx / rootRect.height : 1;
  const EMU_PER_INCH = 914400;
  const presLayout = pptx?.presLayout || pptx?._presLayout;
  const presWidthEmu = presLayout?.width ?? presLayout?._sizeW;
  const presHeightEmu = presLayout?.height ?? presLayout?._sizeH;
  const PPTX_WIDTH_IN =
    typeof presWidthEmu === 'number' ? presWidthEmu / EMU_PER_INCH : 10;
  const PPTX_HEIGHT_IN =
    typeof presHeightEmu === 'number' ? presHeightEmu / EMU_PER_INCH : 5.625;

  const contentWidthIn = rootWidthPx * PX_TO_INCH;
  const contentHeightIn = rootHeightPx * PX_TO_INCH;
...
function isIconElement(node) {
  ...
  const clsLower = typeof clsRaw === 'string' ? clsRaw.toLowerCase() : '';
  if (clsLower.includes('material-icons') || clsLower.includes('material-symbols')) {
    return true;
  }
  ...
  if (tag === 'I' || tag === 'SPAN') {
    ...
    if (typeof cls === 'string') {
      const isMaterial =
        clsLower.includes('material-icons') || clsLower.includes('material-symbols');
      ...
      if (
        ...
        // Material Icons/Symbols are ligature-based, not pseudo-element based.
...
function getIconText(node) {
  ...
  const before = clean(window.getComputedStyle(node, '::before').content);
  const after = clean(window.getComputedStyle(node, '::after').content);
  const combined = `${before}${after}`.trim();
  return combined || '';
}

function resolveIconSource(iconText, node, globalOptions = {}) {
  if (!iconText) return null;

  if (typeof globalOptions.iconResolver === 'function') {
    try {
      const resolved = globalOptions.iconResolver(iconText, node);
  ...
  if (globalOptions.iconMap && globalOptions.iconMap[iconText]) {
    return globalOptions.iconMap[iconText];
  }

  if (globalOptions.iconPathTemplate) {
    const base = (globalOptions.iconBaseUrl || '').replace(/\/$/, '');
    const ext = globalOptions.iconExt || 'svg';
    return globalOptions.iconPathTemplate
      .replace(/\{base\}/g, base)
      .replace(/\{name\}/g, iconText)
      .replace(/\{ext\}/g, ext);
  }

  if (globalOptions.iconBaseUrl) {
    const base = globalOptions.iconBaseUrl.replace(/\/$/, '');
    const ext = globalOptions.iconExt || 'svg';
    return `${base}/${iconText}.${ext}`;
  }
...
function prepareRenderItem(
  ...
  if (node.nodeType === 3) {
    ...
    if (isIconElement(parent)) return null; // Parent icon handles rendering
    ...
    const widthPx = rect.width * config.measureScaleX;
    const heightPx = rect.height * config.measureScaleY;
    const unrotatedW = widthPx * PX_TO_INCH * config.scale;
    const unrotatedH = heightPx * PX_TO_INCH * config.scale;

    const x =
      config.offX +
      (rect.left - config.rootX) * PX_TO_INCH * config.scale * config.measureScaleX;
    const y =
      config.offY +
      (rect.top - config.rootY) * PX_TO_INCH * config.scale * config.measureScaleY;

    const textExpand = getTextBoxExpandIn(config, globalOptions, textContent, style);
    return {
      items: [
        {
          type: 'text',
  ...
  const widthPx = node.offsetWidth || rect.width * config.measureScaleX;
  const heightPx = node.offsetHeight || rect.height * config.measureScaleY;
  const unrotatedW = widthPx * PX_TO_INCH * config.scale;
  const unrotatedH = heightPx * PX_TO_INCH * config.scale;
  ...
  let x =
    config.offX +
    (centerX - config.rootX) * PX_TO_INCH * config.scale * config.measureScaleX -
    unrotatedW / 2;
  let y =
    config.offY +
    (centerY - config.rootY) * PX_TO_INCH * config.scale * config.measureScaleY -
    unrotatedH / 2;
  let w = unrotatedW;
  let h = unrotatedH;
  ...
  if (node.tagName === 'TABLE') {
    const tableData = extractTableData(node, config.scale, config.measureScaleX);
  ...
  if ((node.tagName === 'UL' || node.tagName === 'OL') && !isComplexHierarchy(node)) {
    ...
    liChildren.forEach((child, index) => {
      ...
      const visualIndentPx = (liRect.left - parentRect.left) * config.measureScaleX;
      /*
    ...
    if (listItems.length > 0) {
      ...
      const textExpand = getTextBoxExpandIn(config, globalOptions, listItems, style);
      items.push({
        type: 'text',
        zIndex: zIndex + 1,
        domOrder,
  ...
  if (isIconElement(node)) {
    ...
    if (globalOptions.iconMode === 'text') {
      if (iconText) {
        const textExpand = getTextBoxExpandIn(config, globalOptions, iconText, style);
        return {
          items: [
            {
              type: 'text',
    ...
    const iconSrc = resolveIconSource(iconText, node, globalOptions);
    if (iconSrc) {
      const iconColor = parseColor(style.color);
      const iconTint = iconColor.hex ? { hex: iconColor.hex, opacity: iconColor.opacity } : null;
      const item = {
        type: 'image',
        zIndex,
        domOrder,
  ...
  const bgColorObj = parseColor(style.backgroundColor);
  const readPseudo = (el, pseudo) => {
    const content = window.getComputedStyle(el, pseudo).content;
    if (!content || content === 'none' || content === 'normal' || content === '""') return '';
    return content.replace(/^['"]|['"]$/g, '').trim();
  };
  const pseudoBefore = readPseudo(node, '::before');
  const pseudoAfter = readPseudo(node, '::after');
  const hasDescText =
    (node.textContent && node.textContent.trim().length > 0) ||
    pseudoBefore ||
    pseudoAfter;
  const hasTextLikeDesc = (() => {
    if (hasDescText) return true;
    ...
    while (walker.nextNode()) {
      ...
      if (current.nodeType === 3) {
        if ((current.nodeValue || '').trim()) return true;
        continue;
      }
      if (current.nodeType === 1) {
        const before = readPseudo(current, '::before');
        if (before) return true;
        const after = readPseudo(current, '::after');
  ...
  // --- Optional: Rasterize small pill badges (icon + text) to preserve layout and pseudo-elements ---
  if (globalOptions?.badgeMode === 'image' && bgColorObj.hex) {
    const display = style.display || '';
    const minDim = Math.min(widthPx, heightPx);
    const isPill = borderRadiusValue >= minDim / 2 - 1;
    const isSmall = heightPx <= 48;
    const hasFlex = display.includes('flex');
    const hasPseudoText = !!(readPseudo(node, '::before') || readPseudo(node, '::after'));
    if ((hasDescText || hasPseudoText) && hasFlex && isSmall && isPill) {
      const item = {
        type: 'image',
        zIndex,
        domOrder,
  ...
  const tempBg = bgColorObj;
  const isTxt = isTextContainer(node);

  if (hasPartialBorderRadius && tempBg.hex && !isTxt && !hasTextLikeDesc) {
  ...
  if (hasPartialBorderRadius && isClippedByParent(node) && !hasTextLikeDesc) {
  ...
  if (isText) {
    ...
    if (textParts.length > 0) {
      ...
      textPayload = { text: textParts, align, valign, padding };
    }
  }
  ...
  if (hasGradient || (softEdge && bgColorObj.hex && !isImageWrapper)) {
    ...
    if (textPayload) {
      ...
      const pad = textPayload.padding || [0, 0, 0, 0];
      const textExpand = getTextBoxExpandIn(config, globalOptions, textPayload.text, style);
      let textX = x + pad[3];
      const textY = y + pad[0];
      let textW = Math.max(w - pad[1] - pad[3], 0);
      const textH = Math.max(h - pad[0] - pad[2], 0);
      if (textExpand > 0) {
        if (textPayload.align === 'right') textX -= textExpand;
        else if (textPayload.align === 'center') textX -= textExpand / 2;
        textW += textExpand;
      }
      items.push({
        type: 'text',
    ...
    if (hasPartialBorderRadius && useSolidFill && !textPayload) {
      ...
      if (textPayload) {
        ...
        const needsShape = useSolidFill || hasUniformBorder || hasShadow;
        if (needsShape) {
          items.push({
            type: 'shape',
            zIndex,
