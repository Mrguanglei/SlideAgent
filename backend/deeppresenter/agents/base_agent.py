...
class Agent:
    def __init__(
        ...
        # 优先从 prompts/<ClassName>_prompt.py 加载，fallback 到 roles/<ClassName>.yaml
        role_config = self._load_role_config(config_file)
        self.llm: LLM = config[role_config.use_model]  # type: ignore[index]
    ...
    def _load_role_config(self, config_file: str | None) -> RoleConfig:
        """优先从 prompts/<ClassName>_prompt.py 加载，fallback 到 roles/<ClassName>.yaml"""
        # 1. 尝试 Python 模块
        module_name = f"deeppresenter.prompts.{self.name.lower()}_prompt"
        try:
            mod = importlib.import_module(module_name)
            if hasattr(mod, "ROLE_CONFIG"):
                info(f"Loaded role config from Python module: {module_name}")
                return mod.ROLE_CONFIG
        ...
        if not yaml_path.exists():
            raise FileNotFoundError(
                f"Role config not found. Tried module '{module_name}' and file '{yaml_path}'"
            )
        with open(yaml_path, encoding="utf-8") as f:
            config_data = yaml.safe_load(f)
        info(f"Loaded role config from YAML: {yaml_path}")
            ...
            if len(arguments) == 0:
                ...
                try:
                    ...
                    if t.function.name == "finalize":
                        ...
                        finish_id = t.id
                        # 尝试从参数中获取outcome，如果没有则稍后从工具返回结果中提取
        ...
        if finish_id is not None:
            for obs in observations:
                if obs.tool_call_id == finish_id:
                    # 如果outcome为None，尝试从工具返回结果中提取
                    if outcome is None:
                        try:
                            result = json.loads(obs.text) if isinstance(obs.text, str) else obs.text
                            if isinstance(result, dict) and "outcome" in result:
                                outcome = result["outcome"]
                            elif isinstance(result, dict) and "workspace" in result:
                                # 如果没有outcome字段，使用workspace字段
                                outcome = result["workspace"]
                        except:
                            pass

                    # 如果找到了outcome，返回它；否则返回整个返回值
                    if outcome and obs.text == outcome:
                        info(f"{self.name} Agent finished with result: {obs.text}")
                        return obs.text
                    elif outcome:
                        info(f"{self.name} Agent finished with result: {outcome}")
                        return outcome
                    else:
                        info(f"{self.name} Agent finished with result: {obs.text}")
                        return obs.text
